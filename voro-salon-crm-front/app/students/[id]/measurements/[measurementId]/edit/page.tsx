...
export default function EditMeasurementPage() {
  ...
  const { fetchMeasurementById, updateMeasurement, loading, error } = useMeasurements()
  ...
  useEffect(() => {
    if (studentId && measurementId) {
      fetchMeasurementById(studentId, measurementId).then((data) => {
        if (data) {
          setMeasurement(data)
          setFormData({
  ...
  }, [studentId, measurementId, fetchMeasurementById])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const measurementData: MeasurementDto = {
      ...
      createdAt: measurement?.createdAt ?? new Date(),
    }

    const result = await updateMeasurement(studentId, measurementId, measurementData)
    if (result) {
  ...
  return (
    <AuthGuard requiredRoles={["Trainer"]}>
      <Loading isLoading={!measurement || loading}></Loading>
        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-balance">Editar Medição</h1>
                <p className="text-muted-foreground">Atualize as medidas corporais do aluno</p>
              </div>
            </div>
          </div>
          ...
          <Card className="border-border/50 shadow-lg">
            <CardHeader className="space-y-1 pb-6">
              ...
              <CardDescription>Atualize as medidas corporais do aluno</CardDescription>
            </CardHeader>
            <CardContent>
