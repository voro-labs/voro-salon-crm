    ...
    <SafeAreaView className="flex-1 bg-zinc-50" edges={[]}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable onPress={() => router.push(`${rootPath}/services/${item.id}` as any)} className="bg-white rounded-2xl p-4 mb-2 border border-zinc-100 flex-row items-center gap-3 active:bg-zinc-50">
              ...
              <View className="flex-1">
                ...
                <View className="flex-row gap-3 mt-1">
                  ...
                  {item.price != null && <Text className="font-bold text-sm" style={{ color: primaryColor }}>R$ {item.price.toFixed(2)}</Text>}
                </View>
              </View>
