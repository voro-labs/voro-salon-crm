...
namespace VoroSalonCrm.API.Controllers
{
    ...
    public class PublicBookingController(IPublicBookingService service) : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAvailability([FromQuery] string tenantSlug, [FromQuery] DateTime date, [FromQuery] Guid? serviceId, [FromQuery] Guid? employeeId, [FromQuery] List<Guid>? serviceIds)
        {
            try
            {
                var result = await _service.GetAvailableSlotsAsync(tenantSlug, date, serviceId, employeeId, serviceIds);
